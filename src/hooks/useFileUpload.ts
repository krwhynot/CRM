...
export const useFileUpload = (): UseFileUploadReturn => {
  ...
  const validateRow = useCallback(
    (row: CsvRow): string[] => {
      ...
      if (row['priority-focus']?.trim()) {
        ...
        if (
          !PRIORITY_VALUES.includes(priorityValue.toUpperCase() as PriorityValue) &&
          !PRIORITY_MAPPING[priorityValue.toLowerCase()]
        ) {
          errors.push(
            `Priority "${priorityValue}" not recognized. Will default to 'C'. Expected: ${Object.keys(PRIORITY_MAPPING).slice(0, 8).join(', ')}, etc.`
          )
        }
      }
  ...
  const preprocessCSVData = useCallback(
    (allRows: string[][]): { headerRow: string[]; dataRows: string[][] } => {
  ...
  const findHeaderRow = useCallback((allRows: string[][]): number => {
    let bestRowIndex = 0
    let maxNonEmptyCells = 0

    // Look at first 10 rows to find the one with the most content
    for (let i = 0; i < Math.min(10, allRows.length); i++) {
      const nonEmptyCells = allRows[i].filter(
        (cell) =>
          cell &&
          cell.trim().length > 0 &&
            ...
            try {
              ...
              if (!headerRow.length || !dataRows.length) {
                setUploadState((prev) => ({
                  ...
                  isUploading: false,
                  error:
                    'No valid headers or data found in CSV file. Please check your file format.',
                }))
                return
              }
              ...
              const processedHeaders = headerRow.map((header, index) => {
                ...
                if (normalized !== trimmed) {
                  ...
                  const mappingKey = Object.keys(EXCEL_FIELD_MAPPINGS).find(
                  ...
                  if (errors.length === 0) {
                    ...
                    if (!transformedRow.type) {
                      if (
                        ...
                        const noteText =
                          'Import notes: Type could not be determined from data, assigned as Unknown'
                        transformedRow.import_notes = transformedRow.import_notes
                          ? `${transformedRow.import_notes}; ${noteText}`
                          : noteText
                      }
