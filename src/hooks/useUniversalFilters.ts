...
import { calculateDateRange, getDateRangeDescription, getMemoizedDateRange } from '@/lib/date-range-utils'
...
export const useUniversalFilters = (
  ...
  // Validate filters
  const validateFilters = useCallback((filtersToValidate: UniversalFilterState): FilterValidationResult => {
  ...
  const handleFiltersChange = useCallback((newFilters: UniversalFilterState) => {
  ...
  const updateFilter = useCallback(<K extends keyof UniversalFilterState>(
    field: K, 
  ...
  const updateFocus = useCallback((focus: FocusType) => {
    updateFilter('focus', focus)
  }, [updateFilter])

  const updateQuickView = useCallback((preset: QuickViewType | 'none') => {
    updateFilter('quickView', preset)
  }, [updateFilter])

  // Quick view preset functions
  const applyQuickView = useCallback((preset: QuickViewType) => {
  ...
  const computed = useMemo((): ComputedFilterProperties => {
    ...
    if (filters.timeRange !== DEFAULT_UNIVERSAL_FILTERS.timeRange) {
      summaryParts.push(getDateRangeDescription(filters.timeRange, 
        filters.dateFrom && filters.dateTo 
          ? { start: filters.dateFrom, end: filters.dateTo } 
          : undefined
      ))
...
export const useFilterChangeHandler = (
  ...
  return useCallback(<K extends keyof UniversalFilterState>(
    field: K,
    value: UniversalFilterState[K]
  ) => {
