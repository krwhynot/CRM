...
interface DebouncedSearchInputProps extends Omit<DebouncedInputProps, 'size'> {
    ...
    return (
      <div className="relative">
        <Input
          ...
          className={cn(`${semanticSpacing.rightPadding.xxl}`, className)}
        />
...
export const DebouncedSearchInput = React.memo<DebouncedSearchInputProps>(
  ({
    ...
    // Icon size classes - Input component handles height via its own variants
    ...
    const paddingClasses = showSearchIcon
      ? `${semanticSpacing.leftPadding.xxl}`
      : `${semanticSpacing.leftPadding.lg}`
    ...
    return (
      <div className="relative">
        ...
        <Input
          ...
          size={size}
          className={cn(paddingClasses, `${semanticSpacing.rightPadding.xxl}`, className)}
        />
...
export const DebouncedFilterInput = React.memo<DebouncedFilterInputProps>(
  ({
    ...
    // Debounced value available if needed for future enhancements
    const _debouncedInputValue = useDebounce(inputValue, debounceMs)
    ...
    return (
      <div
        className={cn(
          `flex flex-wrap ${semanticSpacing.gap.xs} ${semanticSpacing.layoutPadding.xs} border ${semanticRadius.default} focus-within:ring-1 focus-within:ring-ring`,
          className
        )}
      >
        {/* Value tags */}
        {values.map((value, index) => (
          <span
            ...
            className={cn(
              ...
              'inline-flex items-center bg-secondary text-secondary-foreground'
            )}
          >
            {value}
            <button
        ...
        <input
          ...
          className={cn(
            ...
            'flex-1 min-w-20 bg-transparent border-none outline-none placeholder:text-muted-foreground'
          )}
...
export const DebouncedNumericInput = React.memo<DebouncedNumericInputProps>(
  ({ value, onChange, debounceMs = 300, min, max, step: _step = 1, placeholder, className }) => {
...
export function useDebouncedFilters<T extends Record<string, any>>(
  ...
  const updateFilter = React.useCallback(<K extends keyof T>(key: K, value: T[K]) => {
