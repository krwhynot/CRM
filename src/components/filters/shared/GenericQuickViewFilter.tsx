    ...
    <div className={`flex min-w-0 items-center ${semanticSpacing.gap.xs} ${className}`}>
      ...
      <Select value={value} onValueChange={onChange} disabled={isLoading}>
              ...
              {showBadges && isActive && value !== 'none' && selectedOption?.badge && (
                <span
                  className={cn(
                    ...
                    semanticSpacing.leftGap.xxs,
                    'bg-secondary ${semanticSpacing.layoutPadding.xxs} ${semanticSpacing.layoutPadding.xxs}',
        ...
        <SelectContent className="w-64">
          {/* Clear/None Option */}
          <SelectItem value="none" className="flex items-center justify-between">
            <div className={`flex items-center ${semanticSpacing.gap.xs}`}>
              <X className="size-4" />
              <div className="flex flex-col">
                <span className={`${semanticTypography.label}`}>Clear Quick View</span>
                <span className={`${semanticTypography.caption} text-muted-foreground`}>
                  Show all items
                </span>
              </div>
            </div>
...
export const createQuickViewOptions = (
  featureType: 'opportunities' | 'interactions' | 'products' | 'organizations' | 'contacts'
): QuickViewOption[] => {
