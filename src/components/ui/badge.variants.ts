...
export const badgeVariants = cva(
  cn(
    `inline-flex items-center border ${semanticTypography.fontWeight.semibold} transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2`,
    semanticSpacing.badgeDefault,
    semanticRadius.md,
  ...
  ),
  {
    variants: {
      ...
      priority: {
        'a-plus': `border-priority-a-plus bg-priority-a-plus bg-gradient-to-r from-priority-a-plus to-priority-a text-priority-a-plus-foreground ${semanticShadows.lg}`,
    ...
    compoundVariants: [
      {
        priority: 'a-plus',
        orgType: 'customer',
        className: `border-yellow-300 bg-gradient-to-r from-yellow-400 to-orange-500 text-white ${semanticShadows.lg}`,
      },
      {
        priority: ['a-plus', 'a'],
        orgType: 'distributor',
        className: `border-green-300 bg-gradient-to-r from-green-400 to-emerald-500 text-white ${semanticShadows.lg}`,
      },
    ],
    defaultVariants: {
      variant: 'default',
