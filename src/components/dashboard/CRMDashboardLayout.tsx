...
export interface DashboardGridProps extends 
...
export function DashboardGrid({
  ...
  return (
    <div 
      className={cn(
        "grid", 
        dashboardLayoutVariants({ layout, spacing }), 
        className
      )}
      {...props}
    >
...
export function ResponsiveDashboard({
  ...
  const LayoutComponent = layout === 'custom' ? DashboardGrid : DashboardLayouts[
...
export function useDashboardCardSizes() {
  ...
  const getCardSpan = (priority: 'low' | 'medium' | 'high' = 'medium') => {
    ...
    // lg and xl
    switch (priority) {
      case 'high': return { lg: 2, xl: 2 }
      case 'medium': return { lg: 1, xl: 1 }
      case 'low': return { lg: 1, xl: 1 }
    }
  }
