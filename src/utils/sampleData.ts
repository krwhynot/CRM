...
import type { Principal, Product, Opportunity, Interaction } from '@/types/dashboard'
...
const opportunityTitles = [
  'Q1 Restaurant Chain Expansion',
  'New Product Line Introduction',
  'Bulk Purchase Agreement',
  'Seasonal Menu Partnership',
...
export const generateSampleProducts = (): Product[] => [
  ...
  {
    id: 'product-4',
    name: 'Specialty Cheese Selection',
    category: 'Dairy',
    principalId: 'principal-2',
  },
  {
    id: 'product-5',
    name: 'Gourmet Spice Blend',
    category: 'Seasonings',
...
export const generateSampleData = () => {
  ...
  const opportunities: Opportunity[] = Array.from({ length: 25 }, (_, index) => {
    ...
    return {
      ...
      status: ['open', 'closed', 'pending'][Math.floor(Math.random() * 3)] as
        | 'open'
        | 'closed'
        | 'pending',
