...
export function calculateOpportunityMetrics(
  ...
  if (filters?.principalIds?.length) {
    filteredOpportunities = filteredOpportunities.filter(opp => 
      opp.principal_organization_id && filters.principalIds!.includes(opp.principal_organization_id)
    )
  }
  ...
  const byStage = filteredOpportunities.reduce((acc, opp) => {
    acc[opp.stage] = (acc[opp.stage] || 0) + 1
    return acc
  }, {} as Record<OpportunityStage, number>)

  const byPriority = filteredOpportunities.reduce((acc, opp) => {
    if (opp.priority) {
  ...
  const stageValues = filteredOpportunities.reduce((acc, opp) => {
    acc[opp.stage] = (acc[opp.stage] || 0) + (opp.estimated_value || 0)
    return acc
  }, {} as Record<OpportunityStage, number>)

  const priorityValues = filteredOpportunities.reduce((acc, opp) => {
    if (opp.priority) {
...
export function calculatePrincipalMetrics(
  ...
  if (filters?.principalIds?.length) {
    filteredPrincipals = filteredPrincipals.filter(p => 
      filters.principalIds!.includes(p.id)
    )
  }

  // Calculate priority breakdown using size mapping
  const byPriority = filteredPrincipals.reduce((acc, principal) => {
    const priority = mapSizeToPriority(principal.size)
  ...
  const principalsWithActiveOpps = new Set(
    activeOpportunities
      .map(opp => opp.principal_organization_id)
      .filter(Boolean)
  )
  const withActiveOpportunities = principalsWithActiveOpps.size

  // Calculate average opportunities per principal
  const totalOpportunities = opportunities.filter(opp => 
    opp.principal_organization_id && 
    filteredPrincipals.some(p => p.id === opp.principal_organization_id)
  ).length
  const averageOpportunitiesPerPrincipal = filteredPrincipals.length > 0 
    ? totalOpportunities / filteredPrincipals.length 
    : 0
  ...
  const principalValues = filteredPrincipals.map(principal => {
    const principalOpportunities = opportunities.filter(opp => 
      opp.principal_organization_id === principal.id
    )
  ...
  const topByPipelineValue = principalValues
...
export function calculateInteractionMetrics(
  ...
  if (filters?.organizationIds?.length) {
    filteredInteractions = filteredInteractions.filter(interaction => 
      interaction.organization_id && filters.organizationIds!.includes(interaction.organization_id)
    )
  }
  ...
  const recentInteractions = filteredInteractions.filter(interaction => {
    ...
    return interactionDate >= dateRanges.last30Days.from && interactionDate <= dateRanges.last30Days.to
  })
  ...
  const thisMonthInteractions = filteredInteractions.filter(interaction => {
    const interactionDate = safeDate(interaction.interaction_date)
    if (!interactionDate) return false
    return interactionDate >= dateRanges.thisMonth.from && interactionDate <= dateRanges.thisMonth.to
  })

  // Calculate type breakdown
  const byType = filteredInteractions.reduce((acc, interaction) => {
    acc[interaction.type] = (acc[interaction.type] || 0) + 1
    return acc
  }, {} as Record<InteractionType, number>)

  // Calculate average interactions per opportunity
  const averagePerOpportunity = opportunities.length > 0 
    ? filteredInteractions.length / opportunities.length 
    : 0
  ...
  const lastActivityDate = filteredInteractions.length > 0
...
export function calculateGrowthMetrics(
  ...
  const currentPrincipals = currentData.organizations.filter(org => org.type === 'principal').length
  const previousPrincipals = previousData.organizations.filter(org => org.type === 'principal').length
  
  const currentDistributors = currentData.organizations.filter(org => org.type === 'distributor').length
  const previousDistributors = previousData.organizations.filter(org => org.type === 'distributor').length

  return {
    organizationsGrowth: calculateGrowth(currentData.organizations.length, previousData.organizations.length),
    contactsGrowth: calculateGrowth(currentData.contacts.length, previousData.contacts.length),
    opportunitiesGrowth: calculateGrowth(currentData.opportunities.length, previousData.opportunities.length),
    pipelineValueGrowth: calculateGrowth(currentOpportunityMetrics.totalPipelineValue, previousOpportunityMetrics.totalPipelineValue),
    interactionsGrowth: calculateGrowth(currentData.interactions.length, previousData.interactions.length),
