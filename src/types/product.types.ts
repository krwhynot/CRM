...
const baseProductSchema = z.object({
  ...
  category: z
    .enum(PRODUCT_CATEGORIES as readonly [string, ...string[]], {
  ...
  sku: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 100,
    { message: 'SKU must be 100 characters or less' }
  ),

  unit_of_measure: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 50,
    { message: 'Unit of measure must be 50 characters or less' }
  ),

  unit_cost: ZodTransforms.nullableNumber.refine(
    (val) => val === null || val > 0,
    { message: 'Unit cost must be positive' }
  ),

  list_price: ZodTransforms.nullableNumber.refine(
    (val) => val === null || val > 0,
  ...
  storage_requirements: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 500,
    { message: 'Storage requirements must be 500 characters or less' }
  ),

  specifications: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 1000,
...
const newPrincipalSchema = z.object({
  ...
  principal_segment: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 100,
    { message: 'Principal segment must be 100 characters or less' }
  ),

  principal_phone: ZodTransforms.nullableString.refine(
    (val) => !val || val.length <= 50,
    { message: 'Phone must be 50 characters or less' }
  ),

  principal_email: ZodTransforms.nullableEmail.refine(
    (val) => !val || val.length <= 255,
    { message: 'Email must be 255 characters or less' }
  ),

  principal_website: ZodTransforms.nullableUrl.refine(
    (val) => !val || val.length <= 255,
...
export const productZodSchema = z.intersection(
  baseProductSchema,
  z.discriminatedUnion('principal_mode', [
    existingPrincipalSchema,
    newPrincipalSchema,
  ])
)
...
export const opportunityProductZodSchema = z.object({
  ...
  unit_price: ZodTransforms.nullableNumber.refine(
    (val) => val === null || val > 0,
