...
export const useImportProgress = (): UseImportProgressReturn => {
  ...
  const importOrganizations = useCallback(async (validRows: TransformedOrganizationRow[]) => {
    ...
    try {
      ...
      console.log(
        `Starting optimized import: ${validRows.length} records in ${batches.length} batches (${batchSize} records per batch)`
      )
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        console.log(
          `Processing atomic batch ${batchIndex + 1}/${batches.length} for session: ${importSessionId}`
        )
        ...
        try {
          ...
          const organizationsToInsert: OrganizationInsert[] = batch.map((row, index) => {
            ...
            // ✅ FIXED: Normalize name for consistent upsert behavior
            // The database will automatically generate name_normalized column
            const normalizedName = row.name.trim()

            return {
              name: normalizedName,
              type: row.type || 'customer', // Ensure type is always set
              priority: (['A', 'B', 'C', 'D'].includes(priority) ? priority : 'C') as
                | 'A'
                | 'B'
                | 'C'
                | 'D',
          ...
          // ✅ OPTIMIZED: Manual upsert logic with proper 406 error handling
          // Uses .maybeSingle() to prevent 406 errors and handles case-insensitive matching
          ...
          for (const org of organizationsToInsert) {
            try {
              // Try to find existing record using normalized name matching
              const { data: existingOrg, error: findError } = await supabase
                .from('organizations')
                .select('id')
                .eq('name_normalized', org.name.toLowerCase().trim())
                .eq('type', org.type)
                .is('deleted_at', null)
                .maybeSingle()
              ...
              if (existingOrg) {
                ...
                const { data: updatedOrg, error: updateError } = await supabase
                  ...
                  .update({
                    ...
                    country: org.country,
                    notes: org.notes,
                    primary_manager_name: org.primary_manager_name,
                    secondary_manager_name: org.secondary_manager_name,
                    import_notes: org.import_notes,
                    import_session_id: org.import_session_id,
                ...
                if (updatedOrg) results.push(updatedOrg)
              } else {
                // Insert new record
          ...
          // Results are now directly from the upsert operation
          ...
          console.log(
            `Batch ${batchIndex + 1} completed in ${batchDuration.toFixed(2)}ms (${recordsPerMs.toFixed(3)} records/ms)`
          )
          ...
          if (error) {
            ...
            if (isCriticalError) {
              ...
              console.error(
                `CRITICAL ERROR - Stopping import for session ${importSessionId}:`,
                criticalError
              )
              ...
              console.log(
                `Non-critical database error in batch ${batchIndex + 1} for session: ${importSessionId}`
              )
            ...
            // ✅ UPDATED: With ignoreDuplicates: false, all records are processed (insert or update)
            // Count all as successfully imported since duplicates are updated rather than skipped
            const processedCount = data?.length || batch.length // Use data length if available, else batch length

            imported += processedCount

            console.log(
              `Batch ${batchIndex + 1} success: ${processedCount} organizations processed (inserted or updated)`
            )

            // No skipped records to track since duplicates are updated instead of skipped
            // The upsert operation handles both new inserts and updates to existing records
          }
        } catch (batchError) {
          // Enhanced batch error handling with critical failure detection
          console.error(
            `Batch ${batchIndex + 1} processing error in session ${importSessionId}:`,
            batchError
          )

          const errorMessage =
            batchError instanceof Error ? batchError.message : 'Unknown processing error'

          // Check if this is a critical processing error
          const isCriticalProcessingError =
          ...
          if (isCriticalProcessingError) {
            ...
            console.error(
              `CRITICAL PROCESSING ERROR - Stopping import for session ${importSessionId}:`,
              criticalError
            )
      ...
      if (processingFailed && criticalError) {
        console.error(
          `Import session ${importSessionId} failed with critical error: ${criticalError}`
        )
      ...
      console.log(
        `Import session ${importSessionId} completed: ${totalRecordsProcessed} records in ${totalDuration.toFixed(2)}ms`
      )
      console.log(
        `Performance: ${recordsPerSecond.toFixed(1)} records/second, ${msPerRecord.toFixed(3)}ms per record`
      )
      console.log(
        `Results: ${imported} imported, ${totalSkipped} duplicates skipped, ${failed} failed`
      )
