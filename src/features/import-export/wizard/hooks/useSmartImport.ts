...
const ENHANCED_FIELD_PATTERNS: FieldPattern[] = [
  ...
  {
    field: 'name',
    patterns: ['name', 'organization', 'company', 'business', 'org', 'firm', 'client', 'customer', 'vendor', 'supplier'],
    confidence: 0.95
  },
  // Contact patterns
  {
...
function fuzzyMatchField(headerName: string, sampleValues: string[] = []): { field: string; confidence: number; reason: string } | null {
  ...
  for (const pattern of ENHANCED_FIELD_PATTERNS) {
    ...
    for (const headerPattern of pattern.patterns) {
      ...
      if (matchingWords.length > 0) {
        const wordMatchRatio = matchingWords.length / Math.max(headerWords.length, patternWords.length)
    ...
    if (pattern.contentMatchers && sampleValues.length > 0) {
      const matchingValues = sampleValues.filter(value => 
        value && pattern.contentMatchers!.some(matcher => matcher.test(value.trim()))
      )
      
      if (matchingValues.length > 0) {
        const contentMatchRatio = matchingValues.length / sampleValues.length
        if (contentMatchRatio > 0.3) { // 30% of values match pattern
...
export function useSmartImport(): UseSmartImportReturn {
  ...
  }, [importState.isImporting, importState.importProgress, importState.importResult, importState.error])

  // Transform AI-mapped data to database format
  const transformDataForImport = useCallback((parsedData: ParsedData, fieldMappings: SmartFieldMapping[]): TransformedOrganizationRow[] => {
    ...
    return parsedData.rows.map(row => {
      ...
      Object.entries(mappingDict).forEach(([csvHeader, crmField]) => {
        ...
        if (value && crmField in transformedRow) {
          if (crmField === 'type') {
            ...
            } else if (['b', 'medium-high', 'priority b', 'above average', '2'].includes(normalizedPriority)) {
              transformedRow.priority = 'B'
            } else if (['c', 'medium', 'priority c', 'standard', 'average', '3'].includes(normalizedPriority)) {
              transformedRow.priority = 'C'
            } else if (['d', 'low', 'priority d', 'minimal', '4'].includes(normalizedPriority)) {
      ...
      if (!transformedRow.name) {
        ...
        transformedRow.name = row['Organizations'] || row['Organization Name'] || row['Company'] || row['name'] || 'Unknown Organization'
      }
            ...
            try {
              ...
              function findHeaderRow(rows: string[][]): { headerRow: string[], headerIndex: number } {
                ...
                rows.forEach((row, index) => {
                  ...
                  if (nonEmptyCells > maxNonEmptyCells && nonEmptyCells > 3) { // Need at least 4 columns
              ...
              const preConfiguredMappings = processedHeaders.map((header) => {
                ...
                const sampleValues = validRows.slice(0, 5).map(row => row[header]).filter(Boolean)
                ...
                if (!fuzzyMatch || fuzzyMatch.confidence < 0.7) {
                  ...
                  const partialMatch = !exactMatch ? Object.entries(COMMON_HEADER_MAPPINGS)
                    .find(([key]) => normalizedHeader.includes(key.toLowerCase()) || key.toLowerCase().includes(normalizedHeader))
                    ?.[1] : null
                  legacyMatch = exactMatch || partialMatch
                }
                
                const bestMatch = fuzzyMatch && fuzzyMatch.confidence >= 0.7 ? fuzzyMatch : 
                                 legacyMatch ? { field: legacyMatch, confidence: 0.85, reason: 'Legacy mapping' } : null
                ...
                return {
                  ...
                  status: bestMatch?.confidence >= 0.95 ? 'auto' : 
                         bestMatch?.confidence >= 0.7 ? 'needs_review' : 'needs_review' as const,
                }
              })
