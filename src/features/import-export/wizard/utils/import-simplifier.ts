 ...
 */
export function detectCriticalIssues(data: ParsedData, mappings: SmartFieldMapping[]): CriticalIssue[] {
  const issues: CriticalIssue[] = []
  
  // Find organization name field
  const orgNameMapping = mappings.find(m => 
    m.crmField === 'name' || 
    m.csvHeader.toLowerCase().includes('organization')
  )
  
  if (orgNameMapping) {
    // Check for missing organization names
    const emptyOrgNames = data.rows.filter(row => 
      !row[orgNameMapping.csvHeader] || 
      row[orgNameMapping.csvHeader].trim() === ''
    ).length
...
export function calculateImportConfidence(mappings: SmartFieldMapping[]): number {
  ...
  const hasOrgName = mappings.some(m => 
    m.crmField === 'name' && m.status !== 'skipped'
  )
