  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-success">{results.summary.validRows}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-warning">{results.summary.warningRows}</div>
      ...
      {results.overallQuality && (
        <Alert className="border-primary/20 bg-primary/5">
          ...
          <AlertDescription>
            ...
            {results.overallQuality >= 0.6 &&
              results.overallQuality < 0.8 &&
              ' - Good data quality with minor issues.'}
            {results.overallQuality < 0.6 &&
              ' - Consider reviewing and cleaning your data before import.'}
          </AlertDescription>
        </Alert>
      )}
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    {!row.isValid && <Badge variant="destructive">Invalid</Badge>}
                    {row.confidence && (
...
function DuplicateResults({ results }: { results: DuplicateDetectionResponseType }) {
  const getSuggestionBadge = (action: string) => {
    switch (action) {
      case 'merge':
        return <Badge className="bg-primary/10 text-primary">Merge</Badge>
      case 'keep_first':
        return <Badge className="bg-success/10 text-success">Keep First</Badge>
      case 'keep_last':
        return <Badge className="bg-purple-100/50 text-purple-700">Keep Last</Badge>
      default:
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-success">{results.summary.uniqueRows}</div>
            <div className="text-sm text-slate-600">Unique Rows</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-warning">{results.summary.duplicateGroups}</div>
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant="outline">{Math.round(group.confidence * 100)}% match</Badge>
  ...
  return (
    <div className={cn('space-y-6', className)}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {duplicateResults && (
              ...
              >
                {hasDuplicates
                  ? `⚠️ ${duplicateResults.summary.totalDuplicates} Duplicates`
                  : '✅ No Duplicates'}
              </Badge>
            )}
          </div>
      ...
      <Tabs defaultValue="preview" className="space-y-4">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="validation" disabled={!validationResults}>
            Validation{' '}
            {validationResults &&
              `(${validationResults.summary.errorRows + validationResults.summary.warningRows})`}
          </TabsTrigger>
        ...
        <TabsContent value="preview" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>First 10 rows of your data with mapped field names</CardDescription>
            </CardHeader>
            <CardContent>
              <DataPreviewTable data={parsedData} fieldMappings={fieldMappings} maxRows={10} />
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="mb-4 text-sm text-slate-600">
                  Run AI validation to check your data quality
                </p>
          ...
          {duplicateResults ? (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="mb-4 text-sm text-slate-600">
                  Check for potential duplicates in your data
                </p>
        ...
        <TabsContent value="mapping">
          <Card>
            <CardHeader>
              ...
              <CardDescription>How your CSV columns are mapped to CRM fields</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
      ...
      {mappedFieldsCount > 0 && (
        <Alert
          className={
            hasErrors ? 'border-destructive/20 bg-destructive/5' : 'border-success/20 bg-success/5'
          }
        >
          <AlertDescription className="flex items-center">
            {hasErrors ? (
              ...
              <>
                ...
                {hasDuplicates &&
                  ` ${duplicateResults?.summary.totalDuplicates} duplicates detected.`}
              </>
            )}
          </AlertDescription>
        </Alert>
