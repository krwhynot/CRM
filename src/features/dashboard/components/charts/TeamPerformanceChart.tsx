...
export const TeamPerformanceChart = React.memo(({ data, loading }: TeamPerformanceChartProps) => {
  if (loading) {
    return (
      ...
      return (
        <div className="rounded-lg border bg-background px-3 py-2 shadow-lg">
          <div className="mb-1 flex items-center gap-2">
            ...
            {teamMember.rank === 1 && <Crown className="size-3 text-warning" />}
          ...
          <p className="mt-1 border-t pt-1 text-sm font-medium">Total Score: {total}</p>
        </div>
      )
    }
    return null
