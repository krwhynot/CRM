...
export const ActivityItemComponent: React.FC<ActivityItemProps> = ({
  ...
  const InteractionIcon =
    ...
    INTERACTION_TYPE_ICONS[item.interactionType as keyof typeof INTERACTION_TYPE_ICONS]
      ? INTERACTION_TYPE_ICONS[item.interactionType as keyof typeof INTERACTION_TYPE_ICONS]
      : null
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex items-start justify-between gap-2">
          <h4
            className={`truncate font-medium ${useNewStyle ? 'text-sm text-foreground' : 'text-sm'}`}
          >
            {item.title}
          </h4>
          {item.priority && (
        ...
        <p className="mb-1 line-clamp-2 text-xs text-muted-foreground">{item.description}</p>
