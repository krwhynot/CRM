...
export function ChartsGrid({
  ...
  const transformToLegacyChartData = (dashboardData: DashboardChartDataPoint[]) => {
    return dashboardData.map((item) => ({
      ...
      weekNumber: Math.ceil(
        (item.weekStart.getTime() - new Date(item.weekStart.getFullYear(), 0, 1).getTime()) /
          (7 * 24 * 60 * 60 * 1000)
      ),
  ...
  const finalOpportunityData = opportunityChartData
    ? transformToLegacyChartData(opportunityChartData)
    : chartData
  const finalInteractionData = interactionChartData
    ? transformToLegacyChartData(interactionChartData)
    : chartData
  ...
  if (isLoading) {
    return (
      <div className={`dashboard-grid dashboard-grid-charts ${className || ''}`}>
  ...
  const allCharts = [
      ...
      component: (
        <PrincipalPerformanceChart
          ...
          onPrincipalClick={(principal) => {
            // TODO: Implement navigation to principal detail page
            void principal
          }}
        />
      ),
    },
    {
      ...
      data: finalTeamData,
      component: <TeamPerformanceChart data={finalTeamData} loading={false} />,
    },
    {
    ...
    },
    {
      ...
      component: (
        <PipelineValueFunnel data={pipelineValueFunnelData} loading={false} showValues={true} />
      ),
    },
  ]
      ...
      <div className="relative">
        ...
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            Charts ({currentIndex + 1} of {charts.length})
          </h3>
