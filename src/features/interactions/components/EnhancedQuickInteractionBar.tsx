...
export function EnhancedQuickInteractionBar({
  ...
  return (
    <form
      ...
      className={cn('bg-blue-50/30 border-y space-y-4', isMobile ? 'p-3' : 'p-4', className)}
    >
      {/* Row 1: Type Selection */}
      <div className="space-y-2">
        <span className="text-sm font-medium text-gray-700">Interaction Type</span>
        <div className={cn('flex gap-2 flex-wrap', isMobile && 'grid grid-cols-2 gap-2')}>
          {ENHANCED_INTERACTION_TYPES.map((typeOption) => {
            ...
            return (
              <Button
                ...
                className={cn('flex-1 min-w-0', isMobile && 'h-12 text-sm flex-col gap-1')}
              >
                <Icon className={cn(isMobile ? 'h-4 w-4' : 'h-3 w-3 mr-2')} />
                {typeOption.label}
              </Button>
            )
          })}
          ...
          <div className="flex gap-1">
            {PRIORITY_OPTIONS.map((p) => (
              <Button
                ...
                className={cn('h-8 px-3 font-bold', priority === p.value && p.color.badge)}
              >
                {p.value}
              </Button>
            ))}
      ...
      {/* Row 6: Actions */}
      <div
        className={cn('flex items-center justify-between pt-2', isMobile && 'flex-col space-y-3')}
      >
        <label
          className={cn(
        ...
        <div className={cn('flex gap-2', isMobile && 'w-full')}>
          {onCancel && (
            ...
            >
              <X className={cn(isMobile ? 'h-5 w-5 mr-2' : 'h-4 w-4 mr-1')} />
              Cancel
            </Button>
          )}
          <Button
            ...
            className={isMobile ? 'h-12 flex-1' : ''}
          >
            <Send className={cn(isMobile ? 'h-5 w-5 mr-2' : 'h-4 w-4 mr-1')} />
      ...
      {(priority || accountManager) && (
        <div className="flex items-center gap-2 border-t pt-2">
          ...
          {priority && (
            <Badge className={cn('text-xs font-bold', PRIORITY_COLORS[priority].badge)}>
              {priority} Priority
            </Badge>
          )}
          {accountManager && (
